...
- **自発的な実行の禁止**: ユーザーが画像生成を明示的に指示・言及していない状態でこのスキルを使おうとする場合（例: 別タスクの途中で AI が自発的に「アイコンも作っておこう」と判断した場合）は、**実行前に必ずユーザーに許可を求めること。許可なく勝手に画像生成を実行してはならない。** 許可を求める際は AskUserQuestion を使い、生成しようとしている内容・枚数・Codex クォータを消費する旨を伝えたうえで可否を聞くこと
